...
import {
  Zap, Lock, Eye, EyeOff, ArrowRight,
  Building2, ShieldCheck, IndianRupee, ArrowUpRight,
  User, Mail, Phone
} from "lucide-react";
...
export default function Register() {
    ...
    <div className="min-h-screen bg-[#FFF4EC] relative overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex flex-1 items-center justify-center px-6 py-6 gap-10">
        ...
        <motion.div
          ...
          className="bg-white/95 backdrop-blur-xl rounded-[40px] shadow-[0_20px_80px_rgba(0,0,0,0.08)] w-full max-w-[620px] px-16 py-12 flex flex-col"
        >
          ...
          {error && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-xl mb-4 text-sm font-semibold"
            >
              {error}
            </motion.div>
          )}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            {/* Username */}
            <div>
  <label className="text-xl font-semibold text-[#111111] mb-3 block">
    Username
  </label>
  ...
  <div className="relative">
    <input
      ...
      type="text"
      placeholder="JhonDoe"
      className="username-input w-full h-16 rounded-2xl border border-gray-200 bg-white pl-12 pr-11 shadow-sm text-lg text-[#111111] focus:outline-none focus:border-[#FF7A00] focus:ring-4 focus:ring-orange-100 transition-all"
    />
  </div>
  ...
  <input
    ...
    className="w-full h-16 rounded-2xl border border-gray-200 bg-white px-6 shadow-sm text-lg text-[#111111] placeholder:text-gray-400 placeholder:font-normal focus:outline-none focus:border-[#FF7A00] focus:ring-4 focus:ring-orange-100 transition-all"
  />
  ...
  />
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#FF7A00] transition-colors">
            ...
            {/* Confirm Password */}
            <div>
              <label className="text-xl font-semibold text-[#111111] mb-3 block">Confirm Password</label>
              <div className="relative">
                <input
                  {...register("password2", {
                  ...
                  className="w-full h-16 rounded-2xl border border-gray-200 bg-white px-6 pr-14 shadow-sm text-lg text-[#111111] placeholder-gray-300 focus:outline-none focus:border-[#FF7A00] focus:ring-4 focus:ring-orange-100 transition-all"
                />
                <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#FF7A00] transition-colors">
                  {showConfirm ? <EyeOff size={20} /> : <Eye size={20} />}
                </button>
              </div>
              {errors.password2 && <p className="text-red-500 text-xs font-semibold mt-2">{errors.password2.message}</p>}
            </div>
            ...
            <motion.button
              ...
              type="submit"
              disabled={loading}
              className="w-full h-16 rounded-2xl bg-gradient-to-r from-[#FF7A00] via-[#FF6A00] to-[#FF4500] text-white text-2xl font-bold shadow-[0_20px_40px_rgba(255,122,0,0.35)] transition-all duration-300 disabled:opacity-60 flex items-center justify-center gap-2 mt-2"
            >
              {loading ? (
            ...
            <p className="text-center text-lg text-gray-500 pt-2">
              ...
              <Link to="/login" className="font-black text-[#FF7A00] hover:text-[#FF5C00] transition-colors inline-flex items-center gap-1">
                Sign in <ArrowUpRight size={16} className="inline" />
              </Link>
            </p>
          </form>
        </motion.div>
